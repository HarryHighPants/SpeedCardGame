...
public class BotService : IBotService
{
	...
	public async Task RunBot(int playerIndex, BotType botType, BotActionHandler actionHandler, CancellationToken cancellationToken)
	{
		var botData = BotConfigurations.GetBot(botType);
		await SeedBot(botData);
		var bot = new Bot(playerIndex, botData);
		_ = RunBot(bot, actionHandler, cancellationToken).ContinueWith((task, _) =>
		{
	...
	private async Task RunBot(Bot bot, BotActionHandler actionHandler, CancellationToken cancellationToken)
	{
		...
		{
			var move = BotRunner.GetMove(gameEngine.Checks, actionHandler.GetGameState(), bot.PlayerIndex,
				centerPilesAvailable);
			if (move.Success)
			{
				await (move.Data.Type switch
				{
					MoveType.PlayCard => actionHandler.PlayCardAction(new(move.Data.CardId.Value,
