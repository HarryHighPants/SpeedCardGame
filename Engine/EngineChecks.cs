...
public class EngineChecks
{
	...
	public bool MustTopUp(GameState gameState) =>
		!gameState.Players.Any(
			p => PlayerHasPlay(gameState, p.Id).Success
			     && CanPickupFromKitty(gameState, p.Id).Success);

	public bool AllPlayersRequestingTopUp(GameState gameState) => gameState.Players.All(p => p.RequestingTopUp);

	public Result<(Card card, int centerPile)> PlayerHasPlay(GameState gameState,
		int playerId,
