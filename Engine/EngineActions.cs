...
public class EngineActions
{
    public GameState NewGame(List<string>? playerNames = null, Settings? settings = null)
    {
        ...
        var gameState = new GameState
        {
            ...
            CenterPiles = new List<CenterPile>
            {
    ...
    private GameState UpdateLastMove(GameState gameState, Move data)
    {
        ...
        var moveDescription = data.GetDescription(
            newGameState,
        ...
        {
            var playerIndex = gameState.Players.IndexOf(
                gameState.Players.FirstOrDefault(p => p.Id == data.PlayerId.Value)
            );
            if (playerIndex != -1)
            {
    ...
    public GameState RequestTopUp(GameState gameState, int playerId)
    {
        ...
        var newPlayers = gameState.Players
            .ReplaceElementAt(gameState.Players.IndexOf(player), newPlayer)
            .ToImmutableList();
    ...
    public Result<GameState> TopUp(GameState gameState)
    {
        ...
        var newCenterPiles = newGameState.CenterPiles
            .Select(
                (pile, i) =>
                    new CenterPile
                    {
                        Cards = pile.Cards
                            .Append(state.Players[i].TopUpCards.Last())
                            .ToImmutableList()
                    }
            )
            .ToImmutableList();
        ...
        var newPlayers = newGameState.Players
            .Select(
                player =>
                    player with
                    {
                        RequestingTopUp = false,
                        TopUpCards = player.TopUpCards.SkipLast(1).ToImmutableList()
                    }
            )
            .ToImmutableList();
    ...
    public GameState SetMustTopUp(GameState gameState, bool mustTopUp) =>
        gameState with
        {
            MustTopUp = mustTopUp
        };
    ...
    public Result<GameState> ReplenishTopUpCards(GameState gameState)
    {
        ...
        var combinedCenterPiles = gameState.CenterPiles
            .SelectMany(cp => cp.Cards.ToList())
            .ToList();
        ...
        var newPlayers = gameState.Players
            .Select(
                player =>
                    player with
                    {
                        TopUpCards = combinedCenterPiles.PopRange(topUpPileSize).ToImmutableList()
                    }
            )
            .ToImmutableList();
        ...
        return Result.Successful(
            ...
            {
                Players = newPlayers,
                CenterPiles = newCenterPiles.ToImmutableList()
            }
        );
    }
    ...
    public GameState PickupCard(GameState gameState, int playerId)
    {
        ...
        var newPlayers = newGameState.Players
            .ReplaceElementAt(newGameState.Players.IndexOf(player), newPlayer)
            .ToImmutableList();
        newGameState = newGameState with { Players = newPlayers };

        // Add the move to the history
        newGameState = UpdateLastMove(
            newGameState,
    ...
    public GameState PlayCard(GameState gameState, Card card, int centerPileIndex)
    {
        ...
        var newCenterPiles = newGameState.CenterPiles
            .ReplaceElementAt(
                ...
                new CenterPile
                {
                    Cards = gameState.CenterPiles[centerPileIndex].Cards
                        .Append(card)
                        .ToImmutableList()
                }
            )
        ...
        var (playerWithCard, playerIndexWithCard) = newGameState.Players
            .IndexTuples()
            .First(p => p.item.HandCards.Contains(card));
        ...
        var newPlayers = newGameState.Players
            .ReplaceElementAt(playerIndexWithCard, newPlayer)
            .ToImmutableList();
        newGameState = newGameState with { Players = newPlayers };
        ...
        newGameState = UpdateLastMove(
            newGameState,
            new Move(MoveType.PlayCard, playerWithCard.Id, card.Id, centerPileIndex)
        );
        return newGameState;
    }
